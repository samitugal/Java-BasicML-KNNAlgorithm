...
public class Money {
    ...
    public double compareResults(ArrayList<Money> denek , ArrayList<Money> ortam , int k ){
        ...
        for(int i = 0 ; i<denek.size();i++){
            UserMoney userMoney = new UserMoney(denek.get(i).varyans,denek.get(i).carpiklik,denek.get(i).basiklik,denek.get(i).entropi,denek.get(i).tur);
            ArrayList<Double> distanceList = new ArrayList<Double>();
            for(int z = 0 ; z<ortam.size();z++){
                double result = userMoney.distanceCalculator(ortam,userMoney,z);
        ...
        double basarı_orani = (True/(True+False));
        return basarı_orani;
    }
